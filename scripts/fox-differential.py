...
FUR_EYE = {"kid": (233, 117, 50, 255), "adult": (250, 132, 40, 255)}
# 嘴区周边是奶油口鼻（bighead 口鼻区 kid=#e3c3a0 / adult=#fce3b3）
CREAM_MUZZLE = {"kid": (227, 195, 160, 255), "adult": (252, 227, 179, 255)}
...
FOX_ZZZ_SMALL = {"kid": [(31, 4, 4), (27, 9, 2)], "adult": [(38, 6, 4), (34, 11, 2)]}  # v7.2: 两个Z（大+小，错开）
...
def render_pose(stage, eyes="open", mouth="smile", tear=None,
                blush=False, bubble=None, zzz=False):
...
if __name__ == "__main__":
    ...
    ALL_POSES = [
        ...
        ("walk-0",    dict(eyes="open",    mouth="smile")),
        ("walk-1",    dict(eyes="open",    mouth="smile")),
        ("walk-2",    dict(eyes="open",    mouth="smile")),
        ("walk-3",    dict(eyes="open",    mouth="smile")),
        ("walk-4",    dict(eyes="open",    mouth="smile")),
        ("walk-5",    dict(eyes="open",    mouth="smile")),
        ("walk-6",    dict(eyes="open",    mouth="smile")),
        # wash 洗澡 2 帧（泡泡左右）
        ("wash-0",    dict(eyes="open",    mouth="smile", bubble="r")),
        ("wash-1",    dict(eyes="open",    mouth="smile", bubble="l")),
    ]
    for stage in ("kid", "adult"):
        for name, kw in ALL_POSES:
            img = render_pose(stage, **kw)
            path = out / f"fox-{stage}-v2-{name}.png"
            img.save(path)
        print(f"  {stage}: wrote {len(ALL_POSES)} poses to {out}/")
    print(f"\nAll {len(ALL_POSES)*2} frames written to assets/sprites/ (v7.5 bighead).")
