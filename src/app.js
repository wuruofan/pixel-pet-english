...
(function () {
  ...
  */
  ...
  function renderLearn(v) {
    v = v || $('#view');
    v.innerHTML = '';
    /* 顶部 tab 直接点学单词时若今日关卡已完成，默认进 browse 让用户自由翻 */
    if (learnMode === 'today' && learnQueue.length && learnPos >= learnQueue.length) {
      learnMode = 'browse';
    }
    if (learnMode === 'today') return renderLearnToday(v);
    return renderLearnBrowse(v);
  }
  /* 今日关卡模式：从 learnQueue 顺序展示，需要跟读 3 次命中才过关 */
  function renderLearnToday(v) {
    if (!learnQueue.length) { learnQueue = buildTodayQueue(); learnPos = 0; }
    if (learnPos >= learnQueue.length) { renderLearnDone(); return; }
    var word = learnQueue[learnPos];
    var supported = learnRecognitionSupported();

    /* 顶部进度条：本课 N 个词 + 当前 hits/3 */
    var header = el('div', 'card');
    header.innerHTML =
      '<div class="row" style="justify-content:space-between;align-items:center">' +
        '<span class="muted">今日新词 · 第 ' + (learnPos + 1) + ' / ' + learnQueue.length + ' 个</span>' +
        '<button class="pill pill-btn" id="learn-browse">📚 翻词库</button>' +
      '</div>' +
    ...
    /* 底部跟读区：识别按钮 + 命中计数；不支持识别时显示自评勾选 */
    var mic = el('div', 'card');
    if (supported) {
      mic.innerHTML =
        '<div class="row" style="justify-content:space-between;align-items:center">' +
          '<div>' +
            '<div style="font-weight:900">🎤 按住说话，读出 "' + esc(word) + '"</div>' +
            '<div class="muted" style="font-size:11.5px;margin-top:3px">点一下开始 · 命中会自动停止 · 最多 8 秒</div>' +
          '</div>' +
          '<div id="hits-pill" class="pill" style="font-size:14px">' + learnHits + ' / 3</div>' +
        '</div>' +
        '<div class="row" style="gap:9px;margin-top:12px">' +
          '<button class="speak-btn big" id="mic-btn" aria-label="开始跟读" style="background:var(--brand)">🎤</button>' +
          '<div style="flex:1;display:flex;flex-direction:column;gap:7px;justify-content:center">' +
            '<div class="hits-row">' + hitsHtml(learnHits) + '</div>' +
            '<div class="row" style="gap:6px">' +
              '<button class="btn ghost xs" id="mic-skip" style="flex:1">跳过</button>' +
              '<button class="btn xs" id="mic-self" style="flex:1;background:var(--blue);color:#fff;box-shadow:0 3px 0 #2f8ce0">我读过了（自评）</button>' +
            '</div>' +
          '</div>' +
        '</div>';
      v.appendChild(mic);
      $('#mic-btn').onclick = function () {
        if (learnBusy) stopLearnMic();
        else startLearnMic();
      };
      $('#mic-skip').onclick = skipLearnWord;
      $('#mic-self').onclick = function () {
        /* 自评等同于一次命中 */
        if (learnBusy) stopLearnMic();
        onLearnHit();
      };
      ...
      mic.innerHTML =
        '<div style="background:#fff6e9;border:2px solid #ffe2b8;border-radius:14px;padding:12px;font-size:13px;font-weight:700">' +
          '⚠️ 当前浏览器不支持语音识别（iOS Safari 等）。<br>' +
          '请用其他按钮手动确认每次跟读。' +
        '</div>' +
        '<div class="row" style="gap:9px;margin-top:12px">' +
          '<button class="speak-btn big" id="mic-btn-fb" aria-label="我读过了" style="background:var(--green);box-shadow:0 5px 0 var(--green-dk)">✅</button>' +
          '<div style="flex:1;display:flex;flex-direction:column;gap:7px;justify-content:center">' +
            '<div style="font-weight:900">点 ✅ 表示读完一次 "' + esc(word) + '"</div>' +
            '<div class="hits-row">' + hitsHtml(learnHits) + '</div>' +
            '<button class="btn ghost xs" id="mic-skip" style="margin-top:6px">跳过这词</button>' +
          '</div>' +
        '</div>';
      v.appendChild(mic);
      $('#mic-btn-fb').onclick = function () { onLearnHit(); };
      $('#mic-skip').onclick = skipLearnWord;
    }
  }
  ...
  function hitsHtml(n) {
    var s = '';
    for (var i = 0; i < 3; i++) s += '<span class="hit' + (i < n ? ' on' : '') + '"></span>';
    return s;
  }

  /* 词卡的视觉 HTML（不含底部按钮），给 today / browse 共用 */
  function learnWordCardHtml(word) {
    ...
    var phoneBlock = pd.length
      ? '<div class="phonics-blocks" id="wc-phon">' +
        ...
        '</div>'
      : '';
    return '<div class="wc-visual">' + visualHtml(word, 76) + '</div>' +
      ...
      '</div>' +
      '<div class="muted" style="margin-top:6px;font-size:11px">点 🔊 听发音 · 点 🐢 听慢速' +
  ...
  function ensureRecognizer() {
    if (learnRecognizer) return learnRecognizer;
    ...
    r.onresult = function (ev) {
      for (var i = ev.resultIndex; i < ev.results.length; i++) {
        ...
        for (var j = 0; j < res.length; j++) {
          if (spokenMatches(res[j].transcript, learnQueue[learnPos])) {
            onLearnHit();
            /* 命中后立即停掉，避免后续噪音继续计分 */
            try { r.stop(); } catch (e) {}
            return;
    ...
    r.onerror = function () { learnBusy = false; render(); };
    r.onend = function () {
      learnBusy = false;
      /* 没人按下停止 / 也没命中 → 让 UI 回到可点状态 */
      var mic = $('#mic-btn'); if (mic) mic.classList.remove('rec');
    };
    learnRecognizer = r;
    return r;
  }
  ...
  function onLearnHit() {
    learnHits = Math.min(3, learnHits + 1);
    stopLearnMic();
    beep('ok');
    if (learnHits >= 3) finishLearnWord();
    else render();
  }
  ...
  function finishLearnWord() {
    ...
    learnPos++;
    learnHits = 0;
    if (learnPos >= learnQueue.length) renderLearnDone();
    else render();
  }
  ...
  /* 跳过当前词：标记为已见过（避免无限循环），给一半奖励，不计新词 */
  function skipLearnWord() {
    var word = learnQueue[learnPos];
    if (!word) return;
    var st = wstate(word);
    if (st.seen === 0) {
      st.seen = 1; st.lastSeen = Date.now(); st.due = Date.now() + 86400000;
      save();
    }
    gainXp(1, 0, 1);
    learnPos++; learnHits = 0;
    if (learnPos >= learnQueue.length) renderLearnDone();
    else render();
  }
  ...
  function renderLearnDone() {
    ...
    var n = learnQueue.length;
    var xpGained = n * 3;
    var c = el('div', 'card');
    c.style.textAlign = 'center';
    c.innerHTML =
      '<div style="font-size:64px;line-height:1;margin:8px 0 6px">🎉</div>' +
      '<h2 class="section" style="font-size:22px;color:var(--brand-dk)">今日新词通关！</h2>' +
      '<div class="muted" style="margin:6px 0 14px">完成 ' + n + ' 个新词 · 获得 <b>+' + xpGained + ' XP</b> · 宠物 +' + n + ' 🍖</div>' +
      '<div class="row" style="gap:9px;margin-top:14px">' +
        '<button class="btn ghost" id="to-browse" style="flex:1">📚 翻一翻词库</button>' +
        '<button class="btn green" id="to-home" style="flex:1">回首页 🏠</button>' +
      '</div>';
    v.appendChild(c);
    $('#to-browse').onclick = function () { enterBrowseMode(); };
  ...
  function enterBrowseMode() {
    learnMode = 'browse';
    learnIdx = 0;
    learnQueue = []; learnPos = 0; learnHits = 0;
    stopLearnMic();
    render();
  }
  function enterTodayMode() {
    learnMode = 'today';
    learnQueue = buildTodayQueue();
    learnPos = 0; learnHits = 0;
    stopLearnMic();
    render();
  }
