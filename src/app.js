  ...
  */
  ...
  function renderLearnToday(v) {
    ...
    card.innerHTML =
      ...
      '<div class="learn-action">' +
        ...
        '<button class="speak-btn sm" id="s2" aria-label="慢速发音">🐢</button>' +
      '</div>' +
      (supported ? '' :
        '<div class="learn-nomic">⚠️ 这个浏览器不能自动听读，点 ✅ 自己确认就行</div>') +
      (passed || !supported ? '' :
        '<button class="btn ghost xs" id="mic-self" style="margin-top:10px">我读过了（自评）</button>') +
      '<div class="learn-nav row">' +
        '<button class="btn ghost xs' + (swipeBlockReason('prev') ? ' off' : '') + '" id="nav-prev">◀ 上一个</button>' +
        '<span class="muted" style="font-size:11px;text-align:center">' +
          (passed ? '← 左滑下一个 · 右滑上一个 →' : '读满 ' + HITS_GOAL + ' 次才能滑走') +
        '</span>' +
        '<button class="btn ghost xs' + (swipeBlockReason('next') ? ' off' : '') + '" id="nav-next">' +
  ...
  function ensureRecognizer() {
      ...
      for (var i = ev.resultIndex; i < ev.results.length; i++) {
        ...
        for (var j = 0; j < res.length; j++) {
          if (spokenMatches(res[j].transcript, learnQueue[learnPos])) {
            /* 停录音由 onLearnHit 统一负责：这里再 stop 一次会多触发一个 onend，
               被误判成「静音自动断」，弹出不该有的「没听清」。 */
            onLearnHit();
            return;
          }
        }
    ...
    r.onerror = function (ev) {
      ...
      /* 只挑需要孩子/家长动手的错误给出明确提示；其余（no-speech 等）交给
         onend 的通用「没听清」分支，避免一个动作弹两次提示。 */
    ...
    r.onend = function () {
      /* 只有「不是我们主动停的」才说明是超时 / 静音自动断，需要给孩子一个反馈。
         刻意不调 render()：命中后的重绘由 onLearnHit 负责，这里重建 DOM 会把
         用户正点着的按钮换掉，造成连点丢失。 */
      var auto = !learnStopping;
      learnStopping = false;
      learnBusy = false;
      if (learnTimer) { clearTimeout(learnTimer); learnTimer = null; }
      syncMicVisual();
      if (auto) toast('没听清，再试一次吧 🎤');
    };
    learnRecognizer = r;
    return r;
  }
  ...
  function syncMicVisual() {
    ...
    if (st) {
      ...
      st.textContent = learnStatusText(w, hitsFor(learnPos), !!learnPassed[learnPos], learnRecognitionSupported());
    }
  }

  function onLearnHit() {
    /* 已过的词再读：停掉录音、给个方向提示，不再重复计分 */
    if (learnPassed[learnPos]) { stopLearnMic(); beep('ok'); toast('已经过关啦，左滑去下一个词 👉'); return; }
