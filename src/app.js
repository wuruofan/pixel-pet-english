...
(function () {
  ...
  */
  ...
  /* 今日关卡模式：主卡 = 词卡 + 跟读区（同一张卡），左右滑切词；
     次卡 = 翻词库入口。计分只在第 3 次命中那一刻发生，滑动是纯导航。 */
  function renderLearnToday(v) {
    ...
    var card = el('div', 'card wordcard learn-swipe');
    card.innerHTML =
      '<div class="row" style="justify-content:space-between;align-items:center">' +
        '<span class="muted">今日新词 · 第 ' + (learnPos + 1) + ' / ' + learnQueue.length + ' 个</span>' +
        '<span class="pill">' + hits + ' / 3</span>' +
      '</div>' +
      learnWordCardHtml(word) +
      '<div class="wc-divider"></div>' +
      learnSpeakHtml(word, supported, hits, passed) +
      '<div class="learn-nav row">' +
        '<button class="btn ghost xs" id="nav-prev"' + (learnPos > 0 ? '' : ' disabled') + '>◀ 上一个</button>' +
        '<span class="muted" style="font-size:11px">← 左滑下一个 · 右滑上一个 →</span>' +
        '<button class="btn ghost xs" id="nav-next">' +
          (learnPos >= learnQueue.length - 1 ? '完成 ✓' : '下一个 ▶') + '</button>' +
      '</div>';
    v.appendChild(card);
    bindWordCardPlayback(card, word);
    bindLearnSpeak(card);
    bindLearnSwipe(card);
    $('#nav-prev').onclick = learnGoPrev;
    $('#nav-next').onclick = learnGoNext;
  ...
  /* 跟读区：通过后换成通过态（不再显示录音按钮）；不支持识别时走自评勾选 */
  function learnSpeakHtml(word, supported, hits, passed) {
    if (passed) {
      return '<div class="learn-pass">' +
        '<div style="font-size:30px;line-height:1">✅</div>' +
        '<div><b style="font-size:15px">' + esc(word) + ' 通过！</b>' +
        '<div class="muted" style="font-size:11.5px;margin-top:2px">左滑或点「下一个」继续</div></div>' +
      '</div>';
    }
    if (!supported) {
      return '<div style="background:#fff6e9;border:2px solid #ffe2b8;border-radius:14px;padding:10px;font-size:12px;font-weight:700;margin-bottom:9px">' +
          '⚠️ 当前浏览器不支持语音识别，点 ✅ 手动确认每次跟读。' +
        '</div>' +
        '<div class="row" style="gap:10px;align-items:center">' +
          ...
          '<div style="flex:1">' +
            '<div style="font-weight:900;font-size:13px">点 ✅ 表示读完一次 “' + esc(word) + '”</div>' +
            '<div class="hits-row" style="margin-top:6px">' + hitsHtml(hits) + '</div>' +
          '</div>' +
        '</div>';
    }
    return '<div class="row" style="gap:10px;align-items:center">' +
        ...
        '<div style="flex:1">' +
          '<div style="font-weight:900;font-size:13px">读出 “' + esc(word) + '”</div>' +
          '<div class="muted" style="font-size:11px;margin-top:2px">点一下开始 · 命中自动停 · 最多 8 秒</div>' +
          '<div class="hits-row" style="margin-top:6px">' + hitsHtml(hits) + '</div>' +
        '</div>' +
      '</div>' +
      '<button class="btn ghost xs" id="mic-self" style="margin-top:8px;width:100%">我读过了（自评）</button>';
  }
  ...
  function bindLearnSpeak(scope) {
    ...
    if (mic) mic.onclick = function () { if (learnBusy) stopLearnMic(); else startLearnMic(); };
    var self = scope.querySelector('#mic-self');
    if (self) self.onclick = function () { if (learnBusy) stopLearnMic(); onLearnHit(); };
  }

  /* 左右滑切词：纯导航，不影响计分。
     起手落在按钮上 / 拖太久 / 以竖向为主 → 都不算滑动，避免误触和抢页面滚动。 */
  var SWIPE_MIN = 44;
  function bindLearnSwipe(node) {
    var x0 = null, y0 = null, t0 = 0, onBtn = false;
    ...
    node.addEventListener('touchend', function (e) {
      ...
      var sx = dx < 0 ? -dx : dx, sy = dy < 0 ? -dy : dy;
      x0 = null;
      if (onBtn || Date.now() - t0 > 800) return;
      if (sx < SWIPE_MIN || sx < sy * 1.2) return;
      if (dx < 0) learnGoNext(); else learnGoPrev();
    }, { passive: true });
  }
  ...
  function onLearnHit() {
    var h = Math.min(3, hitsFor(learnPos) + 1);
    learnHits[learnPos] = h;
    stopLearnMic();
    beep('ok');
    if (h >= 3) markLearnPassed();
    render();
  }
  ...
  function learnGoPrev() {
    ...
    learnPos--; stopLearnMic(); render();
  }
  /* 桌面端没有触摸：方向键同样映射为「左=下一个 / 右=上一个」 */
  document.addEventListener('keydown', function (e) {
    if (tab !== 'learn' || learnMode !== 'today') return;
    if (e.key === 'ArrowLeft') learnGoNext();
    else if (e.key === 'ArrowRight') learnGoPrev();
  });
